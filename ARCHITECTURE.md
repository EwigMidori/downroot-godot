...
以下原则是后续所有 UI 设计与实现的长期约束：

- 产品规格必须给出唯一结果，不把页面结构、交互路径、组件职责留给程序员临时决策。
- 页面描述必须从页面层一直写到叶子组件层，不能只写“有一个背包面板”或“有一个合成界面”。
- 每个命名组件都必须说明：
  - 它的作用是什么
  - 它显示什么
  - 它如何显示
  - 它在什么状态下显示、隐藏、弱化或高亮
- UI 必须优先依赖图形、颜色、长度、位置、图标轮廓传达信息，文字只作为补充，而不是唯一信息来源。
- 关键玩法反馈必须尽量文盲友好：
  - 状态优先用条形、颜色和长度表达
  - 物品优先用图标轮廓表达
  - 交互优先用键帽样式和动作图标表达
  - 配方优先用产出图标和材料图标表达
- tooltip 可以作为紧凑界面中的补充信息层，用来承载放不下的名称或补充说明，但不能成为识别对象、判断可否执行、理解当前状态的唯一信息来源。
- 不允许用系统组件冒充正式 UI：
  - 不用 `ProgressBar` 冒充正式条形控件
  - 不用 `ItemList` / `Tree` 冒充正式列表
  - 不用 `Button` / `TextureButton` 冒充正式槽位
  - 不用长文本 `Label` / `RichTextLabel` 冒充复杂 HUD
- 正式 UI 一旦开始设计，就必须固定主路径，不保留“兼容方案”“可选方案”“A 或 B 都可以”的产品层空间。
- Gameplay 提供语义状态，UI 负责把语义状态转换成固定的视觉表达；不要让 gameplay 直接拼最终 HUD 文案。
